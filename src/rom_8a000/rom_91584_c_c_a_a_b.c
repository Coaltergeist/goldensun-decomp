 ...
 * the absolute asset-symbol MSG_LEARN_INNATE_MOVE (defined in asset_ids.sym).
 * (int)&SYM forces gcc-2.96 to materialize it from the literal pool, matching
 * the ROM's `ldr r0, =...` (where stock gcc would emit an inline `mov`). See
 * FP9_PLAN.md / COMPILER_NOTES.md (FP#9). Original model output (literal 0x1e)
 * preserved in the sibling .response file.
 */
...
extern unsigned char MSG_LEARN_INNATE_MOVE;   /* asset_ids.sym, = 0x1e */
...
void Func_8091d94(unsigned int pc, unsigned int move) {
    ...
    _Func_8019908(r5, 1);
    _Func_8019908(r6, 4);
    _Func_801776c((int)&MSG_LEARN_INNATE_MOVE, 3);
}
