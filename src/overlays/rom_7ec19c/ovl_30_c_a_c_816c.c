...
void OvlFunc_962_200816c(int actor_id)
{
    ...
    asm volatile("ldr %0, =0x96f" : "=r"(argument));
    if (__GetFlag(argument)) {
        asm volatile("ldr %0, =0x262c" : "=r"(argument));
        __MessageID(argument);
        __ActorMessage(actor_id, 0);
        asm volatile("b .Lreturn_962_200816c\n\t.align 2, 0\n\t.pool");
    }

    __MessageID(0x25d5);
    __ActorMessage(actor_id, 0);
    asm volatile(".Lreturn_962_200816c:");
}
