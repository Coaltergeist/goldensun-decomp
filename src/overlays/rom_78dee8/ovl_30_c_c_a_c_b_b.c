...
void OvlFunc_895_200870c(void)
{
    ...
    if (actor) {
        ...
        {
            register unsigned int flag asm("r0") = 0x318;
            asm("" : "+r" (flag));
            tile_x = raw_x >> 20;
            __ClearFlag(flag);
        }
        {
            register unsigned int flag asm("r0") = 0x319;
            asm("" : "+r" (flag));
            __ClearFlag(flag);
        }
        if (tile_x == 0x73) {
...
void OvlFunc_895_2008754(void)
{
    ...
    if (actor) {
        ...
        /* Materialize each flag independently in the original call register. */
        {
            register unsigned int flag asm("r0") = 0x31a;
            asm("" : "+r" (flag));
            tile_x = raw_x >> 20;
            __ClearFlag(flag);
        }
        {
            register unsigned int flag asm("r0") = 0x31b;
            asm("" : "+r" (flag));
            __ClearFlag(flag);
        }
        if (tile_x == 0x77) {
