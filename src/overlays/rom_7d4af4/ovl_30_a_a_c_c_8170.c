...
extern int OvlFunc_949_200807c(u8 *actor, u8 *target, int command, int option);

/* 相手役を選び、接触時の動作を更新する */
int OvlFunc_949_2008170(u8 *actor)
{
    ...
    if ((temp & 1) != 0)
        target = __MapActor_GetActor(0x11);
    else
        target = __MapActor_GetActor(0x10);

    if (OvlFunc_949_200807c(actorReg, target, 0x20, 0) == 0) {
        ...
        OvlFunc_949_200807c(actorReg, target, command, option);
    }
